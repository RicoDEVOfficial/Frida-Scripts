	...
	Interceptor.attach(Module.findExportByName('libc.so', 'connect'), {
        onEnter: function(args) {
            if (ntohs(Memory.readU16(args[1].add(2))) === 9339) {
				...
				Memory.writeInt(args[1].add(4), inet_addr(Memory.allocUtf8String(ip))); //ip here
				Memory.writeU16(args[1].add(2), ntohs(port)); // port here i think??
				log(format("redirected to {0}:{1}", ip, port)); // whats this
				setupMessaging();
            }
        }
    });
