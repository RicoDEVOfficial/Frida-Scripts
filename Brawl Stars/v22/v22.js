...
function debug_init(land) { // смска всё идёт нормально. соединение с оператором прервано.
	let dbutton = malloc(300);
	...
	cache.addGemsButton = malloc(300);
...
const Messaging = {
    init: function() {
        ...
        Messaging.onReceive = function() {
            ...
            if (r == 0) {
                console.log("disconnected нахуй!");
...
rpc.exports = {
    init: function(stage, options) {
        ...
        HostPatcher.redirectAddress = "185.104.248.10" // ip here
        //DebugPatcher.enableIsDev();
        HostPatcher.init();
	podload();
    }
